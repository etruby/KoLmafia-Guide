...
void SMiscItemsGenerateResource(ChecklistEntry [int] resource_entries)
{
	...
	if ($item[smut orc keepsake box].available_amount() > 0 && !__quest_state["Level 9"].state_boolean["bridge complete"] && __misc_state["in run"])
		resource_entries.listAppend(ChecklistEntryMake(314, "__item smut orc keepsake box", "inventory.php?which=3&ftext=smut+orc+keepsake+box", ChecklistSubentryMake(pluralise($item[smut orc keepsake box]), "", "Open for bridge building purposes."), 0));
